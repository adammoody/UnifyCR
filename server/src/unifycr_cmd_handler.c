...
*/
static int attach_to_shm(app_config_t *app_config, int app_id, int client_side_id)
{
...
static thrd_ctrl_t *unifycr_rm_thrd_create(int app_id, int client_id)
{
    ...
    if (thrd_ctrl == NULL) {
        LOGERR("Failed to allocate structure for request manager thread for app_id=%d client_id=%d", app_id, client_id);
        return NULL;
    }
    ...
    if (thrd_ctrl->del_req_set == NULL) {
        LOGERR("Failed to allocate read request structure for request manager thread for app_id=%d client_id=%d", app_id, client_id);
        free(thrd_ctrl);
        return NULL;
    }
    ...
    if (thrd_ctrl->del_req_stat == NULL) {
        LOGERR("Failed to allocate delegator structure for request manager thread for app_id=%d client_id=%d", app_id, client_id);
    ...
    if (thrd_ctrl->del_req_stat->req_stat == NULL) {
        LOGERR("Failed to allocate per-delegator structure for request manager thread for app_id=%d client_id=%d", app_id, client_id);
    ...
    if (rc != 0) {
        LOGERR("pthread_mutex_init failed for request manager thread app_id=%d client_id=%d rc=%d (%s)",
    ...
    if (rc != 0) {
        LOGERR("pthread_cond_init failed for request manager thread app_id=%d client_id=%d rc=%d (%s)",
            app_id, client_id, rc, strerror(rc));
    ...
    cli_signature_t *cli_signature = (cli_signature_t *)malloc(sizeof(cli_signature_t));
    ...
    if (rc != 0) {
        LOGERR("pthread_create failed for request manager thread app_id=%d client_id=%d rc=%d (%s)",
            app_id, client_id, rc, strerror(rc));
        pthread_cond_destroy(&(thrd_ctrl->thrd_cond));
        pthread_mutex_destroy(&(thrd_ctrl->thrd_lock));
        free(thrd_ctrl->del_req_stat->req_stat);
